...
class Layers2D(models.Model):
    ...
    imgId = models.ForeignKey("imgs.Imgs", on_delete=models.CASCADE, verbose_name="이미지 아이디")
...
class Layers2D3D(models.Model):
    ...
    projectId = models.ForeignKey('projects.Projects', to_field='id', on_delete=models.CASCADE,
                                  verbose_name='프로젝트 아이디')
    mosaicId = models.ForeignKey('processing.Mosaics', on_delete=models.CASCADE, verbose_name='모자익 아이디')
    ...
    created = models.DateTimeField(auto_now_add=True, null=False, blank=False, verbose_name="레이어 생성 시간")
    modified = models.DateTimeField(auto_now=True, null=False, blank=False, verbose_name="레이어 수정 시간")
...
class Layers3D(models.Model):
    ...
    ObjId = models.ForeignKey('processing.Objects3D', on_delete=models.CASCADE, related_name="layer3d", verbose_name='3D 오브젝트 아이디')
...
class LayerMtls(models.Model):
    ...
    fileDir = models.CharField(max_length=500, null=True, blank=True, verbose_name='mtl 파일 경로')
    projectId = models.ForeignKey('projects.Projects', on_delete=models.CASCADE, verbose_name='프로젝트 아이디')
    layerId = models.ForeignKey('Layers3D', on_delete=models.CASCADE, related_name="layermtl", verbose_name='3D 레이어 아이디')
    size = models.BigIntegerField(default=0, null=False, blank=False, verbose_name='파일 사이즈')
    email = models.ForeignKey('users.Users', to_field='email', on_delete=models.CASCADE, verbose_name='이메일',
                              db_column='email')
...
class LayerTextures(models.Model):
    ...
    projectId = models.ForeignKey('projects.Projects', on_delete=models.CASCADE, verbose_name='프로젝트 아이디')
    layerId = models.ForeignKey('Layers3D', on_delete=models.CASCADE, related_name="layertexture", verbose_name='3D 레이어 아이디')
    size = models.BigIntegerField(default=0, null=False, blank=False, verbose_name='파일 사이즈')
    email = models.ForeignKey('users.Users', to_field='email', on_delete=models.CASCADE, verbose_name='이메일',
                              db_column='email')
