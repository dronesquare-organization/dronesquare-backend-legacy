...
from .serializers import (
    Etc2D3DLayersSerializer,
    EtcDocsSerializer,
    EtcImgsSerializer,
    GCPSerializer,
    NestedLayersSerializer,
    VideoSerializer,
)
...
class VideoViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic()
    @swagger_auto_schema(
        operation_description="비디오 파일 업로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def create(self, request):
        ...
        if type(request.data) != dict:
            ...
            tmppath = (
                settings.MEDIA_ROOT
                + "/{email}/projects/{projectId}/etcTempVideo".format(
                    email=str(email), projectId=request.data["projectId"]
                )
            )
            path = (
                settings.MEDIA_ROOT
            ...
            if (
                ...
                and request.data["allFileLength"] == request.data["index"]
            ):
                onlyfiles = [
                    f
                ...
                if result["sum_of_size"] is not None:
                    ...
                    projects.save()
                videoList = Video.objects.filter(
                    projectId=projects.id, email=users.email
                ).order_by("-uploadDate")
...
class VideoDeleteViewSet(viewsets.ViewSet):
    ...
    def video(self, request, projectId):
        ...
        result = Video.objects.filter(email=email, projectId=projectId).aggregate(
...
class VideoListViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic()
    @swagger_auto_schema(
        operation_description="비디오 리스트 조회",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    @action(
        detail=False, methods=["get"], url_path=r"(?P<projectId>\d+)", url_name="video"
    )
...
class VideoDownloadViewSet(viewsets.ViewSet):
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path=r"(?P<projectId>\d+)/(?P<id>\d+)",
        url_name="video",
    )
        ...
        if video.exists():
            ...
            try:
                ...
                response = HttpResponse(
                    FileWrapper(BytesIO(obj["Body"].read())), content_type=video.ext
                )
...
class EtcImgsViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic()
    @swagger_auto_schema(
        operation_description="기타 이미지 파일 업로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def create(self, request):
        ...
        if type(request.data) != dict:
            ...
            tmppath = (
                settings.MEDIA_ROOT
                + "/{email}/projects/{projectId}/etcTempImg".format(
                    email=str(email), projectId=request.data["projectId"]
                )
            )
            path = settings.MEDIA_ROOT + "/{email}/projects/{projectId}/etcImg".format(
                email=str(email), projectId=request.data["projectId"]
            )
            if request.data["is_Start"] == "true":
                if os.path.exists(tmppath):
            ...
            if (
                ...
                for f in onlyfiles:
                    if str(f).split(".")[-1].lower() in accept:
                        ...
                        fail.append(f)
                etcImgList = EtcImgs.objects.filter(
                    projectId=projects.id, email=users.email
                ).order_by("-uploadDate")
...
class EtcImgsDownloadViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        operation_description="기타 이미지 파일 다운로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def etcImgs(self, request, projectId, id):
        ...
        if etcImgs.exists():
            ...
            try:
                ...
                obj = s3_client.get_object(Bucket=bucket, Key=file_path)
                response = HttpResponse(
                    FileWrapper(BytesIO(obj["Body"].read())), content_type=etcImg.ext
                )
...
class EtcDocsViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic()
    @swagger_auto_schema(
        operation_description="기타 문서 파일 업로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def create(self, request):
        ...
        if type(request.data) != dict:
            ...
            tmppath = (
                settings.MEDIA_ROOT
                + "/{email}/projects/{projectId}/etcTempDoc".format(
                    email=str(email), projectId=request.data["projectId"]
                )
            )
            path = settings.MEDIA_ROOT + "/{email}/projects/{projectId}/etcDoc".format(
                email=str(email), projectId=request.data["projectId"]
            )
            if request.data["is_Start"] == "true":
                if os.path.exists(tmppath):
            ...
            if (
                ...
                for f in onlyfiles:
                    if str(f).split(".")[-1].lower() in accept:
                        ...
                        fail.append(f)
                etcDocList = EtcDocs.objects.filter(
                    projectId=projects.id, email=users.email
                ).order_by("-uploadDate")
...
class EtcDocsDownloadViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        operation_description="기타 문서 파일 다운로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def etcDocs(self, request, projectId, id):
        ...
        if etcDocs.exists():
            ...
            try:
                ...
                obj = s3_client.get_object(Bucket=bucket, Key=file_path)
                response = HttpResponse(
                    FileWrapper(BytesIO(obj["Body"].read())), content_type=etcDoc.ext
                )
...
class NestedLayersViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic()
    @swagger_auto_schema(
        operation_description="레이어 중첩 파일 업로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
            ...
            if (
                ...
                for f in onlyfiles:
                    if str(f).split(".")[-1].lower() in accept:
                        ...
                        fail.append(f)
                nestedLayerList = NestedLayers.objects.filter(
                    projectId=projects.id, email=users.email
                ).order_by("-uploadDate")
...
class NestedLayersUpdateViewSet(viewsets.ViewSet):
            ...
            properties={
                "body": openapi.Schema(
                    ...
                    items=openapi.Schema(
                        ...
                        properties={
                            "id": openapi.Schema(
                                type=openapi.TYPE_INTEGER, description="id"
                            ),
                            "coordSystem": openapi.Schema(
                                type=openapi.TYPE_STRING, description="coordSystem"
                            ),
                        },
                    ),
                    description="array",
    ...
    def enroll(self, request, projectId):
        print(request.data)
        email = str(request.user)
        NestedLayers.objects.filter(email=email, projectId=projectId).update(
            isEnroll=False
        )
...
class NestedLayersDeleteViewSet(viewsets.ViewSet):
    ...
    def deleteNestedLayer(self, request, projectId):
        ...
        nestedLayer = NestedLayers.objects.filter(
            projectId=projectId, email=email, id__in=idSet
        )
...
class NestedLayersDownloadViewSet(viewsets.ViewSet):
    ...
    def NestedLayers(self, request, projectId, id):
        email = str(request.user)
        request.data["email"] = email
        nestedLayers = NestedLayers.objects.filter(
            projectId=projectId, email=email, id=id
        )
        if nestedLayers.exists():
            ...
            try:
                ...
                obj = s3_client.get_object(Bucket=bucket, Key=file_path)
                response = HttpResponse(
                    FileWrapper(BytesIO(obj["Body"].read())),
                    content_type=nestedLayer.ext,
                )
                response["Content-Length"] = nestedLayer.size
                response["Content-Disposition"] = (
                    "inline; filename=%s" % nestedLayer.name
                )
                return response
            except Exception as e:
                print(e, flush=True)
...
class Etc2D3DLayersViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic()
    @swagger_auto_schema(
        operation_description="기타 2D3D 레이어 파일 업로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def create(self, request):
        ...
        if type(request.data) != dict:
            ...
            tmppath = (
                settings.MEDIA_ROOT
                + "/{email}/projects/{projectId}/etcTemp2D3DLayer".format(
                    email=str(email), projectId=request.data["projectId"]
                )
            )
            path = (
                settings.MEDIA_ROOT
                + "/{email}/projects/{projectId}/etc2D3DLayer".format(
                    email=str(email), projectId=request.data["projectId"]
                )
            )
            if request.data["is_Start"] == "true":
            ...
            if (
                ...
                for f in onlyfiles:
                    if str(f).split(".")[-1].lower() in accept:
                        ...
                        fail.append(f)
                etc2D3DLayersList = Etc2D3DLayers.objects.filter(
                    projectId=projects.id, email=users.email
                ).order_by("-uploadDate")
...
class Etc2D3DLayersDeleteViewSet(viewsets.ViewSet):
    ...
    def deleteEtc2D3DLayer(self, request, projectId):
        email = str(request.user)
        idSet = request.data["idSet"]
        etc2D3DLayers = Etc2D3DLayers.objects.filter(
            projectId=projectId, email=email, id__in=idSet
        )
...
class Etc2D3DLayersListViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic()
    @swagger_auto_schema(
        operation_description="기타 2D3D 레이어 파일 리스트 조회",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def getEtc2D3DLayersList(self, request, projectId):
        email = str(request.user)
        request.data["email"] = email
        Etc2D3DLayerList = Etc2D3DLayers.objects.filter(
...
class Etc2D3DLayersDownloadViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        operation_description="기타 2D3D 레이어 파일 다운로드",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def Etc2D3DLayers(self, request, projectId, id):
        ...
        if etc2D3DLayers.exists():
            ...
            if os.path.exists(file_path):
                ...
                response = HttpResponse(
                    FileWrapper(doc), content_type=etc2D3DLayers.ext
                )
                response["Content-Length"] = etc2D3DLayers.size
                response["Content-Disposition"] = (
                    "inline; filename=%s" % etc2D3DLayers.name
                )
                return response
        return Response({"message": "not exist"})
