...
from processing.serializers import MosaicsSerializer, Objects3DJoinSerializer, FastMosaicsSerializer
...
class MosaicsViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="모자익 영상 조회",
                         manual_parameters=[
...
class VoxelViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 지점과 관련된 이미지 찾기",
...
class Objects3DViewerViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="3D Object Viewer 정보 가져오기",
                         manual_parameters=[
                             openapi.Parameter("Authorization", openapi.IN_HEADER, description="Authorization",
                                               type=openapi.TYPE_STRING)])
    @action(detail=False, methods=["get"], url_path=r"(?P<projectId>\d+)")
...
class FastMosaicsViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="간편 모자익 영상 조회",
                         manual_parameters=[
                             openapi.Parameter("Authorization", openapi.IN_HEADER, description="Authorization",
                                               type=openapi.TYPE_STRING),
                             openapi.Parameter("projectId", openapi.IN_PATH, description="PROJECT ID",
