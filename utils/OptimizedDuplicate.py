...
def optimizeDuplicate(params):
    ...
    for imgN in imgList:
        ...
        if flagNext == 1:  # 가장 처음 이미지 리드용 로직
            ...
            im1 = cv2.imdecode(
                n, cv2.IMREAD_COLOR
            )  # 첫번째 이미지 리드 - 읽어온 이미지 파일을 바이너리 형태로 디코딩, Byte 단위
            im1_G = cv2.cvtColor(
                im1, cv2.COLOR_BGR2GRAY
            )  # 첫번째 이미지 리드 - BGR에서 Gray-scale로 컨버팅
            flagNext = 2
        ...
        im2 = cv2.imdecode(
            n, cv2.IMREAD_COLOR
        )  # 두번째 이미지 리드 - 읽어온 이미지 파일을 바리너리 형태로 디코딩, Byte 단위
        im2_G = cv2.cvtColor(
            im2, cv2.COLOR_BGR2GRAY
        )  # 두번째 이미지 리드 - BGR에서 Grey-scale로 컨버팅
