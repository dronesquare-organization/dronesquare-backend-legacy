...
def IOR_F(Img_path):
    ...
    Thresh_num_ORimgs = int(
        num_selectedimg * 0.5
    )  # 2. the number of image pairs with the proper overlapping rate
    ...
    for i in range(num_selectedimg):
        ...
        if match_rate > thresh_min_match_rate and len(good_points) > min_match:
            ...
            Hinv, statusinv = cv2.findHomography(
                image1_kp, image2_kp, cv2.RANSAC, 5.0
            )  # img1 to img2(refer)
        else:
            num_unOR += 1
            continue
        ...
        # projection
        val_pano2 = cv2.warpPerspective(
            img2, H, (width_panorama, height_panorama)
        )  # img2 to img1
        val_pano1 = cv2.warpPerspective(
            img1, Hinv, (width_panorama, height_panorama)
        )  # img1 to img2
