...
def transform_coordinate(
    origin, destination, data, type_of_method
):  # 좌표계 변환 후 GeoJSON 폴리곤 포맷으로 변환
    transformResult = []
    points = []
...
def PointToPoint3Distance(a1, a2):
    if a1[2] == -10000 or a2[2] == -10000:
        return "-"
    return math.sqrt(
        pow((a1[0] - a2[0]), 2) + pow((a1[1] - a2[1]), 2) + pow((a1[2] - a2[2]), 2)
    )
    ...
    for i in range(1, len(arr)):
        ...
        if PointToPoint3Distance(arr[i - 1], arr[i]) == "-":
            ...
            angle.append(
                ...
                * math.acos(
                    float(
                        PointToPoint2Distance(arr[i - 1], arr[i])
                        / PointToPoint3Distance(arr[i - 1], arr[i])
                    )
                )
            )
    measureResult = math.sqrt(
        totalArea[0] * totalArea[0]
        + totalArea[1] * totalArea[1]
        + totalArea[2] * totalArea[2]
    )
    position.append(
        [points[len(arr) - 1][0], points[len(arr) - 1][1], arr[len(arr) - 1][2]]
    )
    return {
        "d3": measureResult if invalid == False else "-",
        "position": position,
...
def calculate3DDistance(arr, points):  # 측정에서 점 사이 거리 측정할 때 사용
    ...
    return {
        "d3": sum(distance) if invalid == False else "-",
        "position": position,
        "segment": {"d3": distance, "elevation": elevation, "angle": angle},
    }
    ...
    if typeOfJson != "Point":
        ...
        if typeOfJson == "Polygon":
            transformResult = transform_coordinate(
                3857,
                int(referencesData.coordSystem),
                inputGeoJson[0]["coordinates"][0],
                "Polygon",
            )
            for coord in inputGeoJson[0]["coordinates"][0]:
                points.append((coord[0], coord[1]))
        else:
            line_transformResult = transform_coordinate(
                3857,
                int(referencesData.coordSystem),
                inputGeoJson[0]["coordinates"],
                "Polygon",
            )
            if len(line_transformResult) < 4:
                ...
                transformResult = [
                    [min(line_x), min(line_y)],
                    [max(line_x), min(line_y)],
                    [max(line_x), max(line_y)],
                    [min(line_x), max(line_y)],
                ]
            else:
        ...
        targetResult = transform_coordinate(
            3857, int(referencesData.coordSystem), [x, y], "Point"
        )
        inputGeoJson[0]["coordinates"] = [transformResult]
        ...
        with rasterio.Env(
            ...
            with rasterio.open(dsm) as file:
                out_image, out_transform = mask(
                    file, inputGeoJson, crop=True, all_touched=True, nodata=-10000
                )
...
def volume_by_bottom_total(
    out_image, minz, gsd
):  # 전체 볼륨 계산(체적 정보) minz - 바닥면 높이 설정값, gsd - Dsm 가준 1픽셀당 실제 거리값
...
def calculateVolume(
    inputGeoJson, referencesData, minz
):  # 체적 정보 계산 메소드, 위에 있는 3개 메소드를 사용하여 계산함
    transformResult = transform_coordinate(
        3857,
        int(referencesData.coordSystem),
        inputGeoJson[0]["coordinates"][0],
    ...
    try:
        ...
        with rasterio.Env(
            ...
            with rasterio.open(dsm) as file:
                out_image, out_transform = mask(
                    file, inputGeoJson, crop=True, nodata=-10000, filled=True
                )

        result = {
            "total": volume_by_bottom_total(out_image[0], minz, referencesData.gsd),
...
def getLocation(
    inputLoc, referencesData
):  # 마우스 포인터 위치에 따른 Z값 계산 메소드 -  Maybe 2초 딜레이 설정되어 있음
    ...
    transformResult = transform_coordinate(
        3857,
        int(referencesData.coordSystem),
        geoJsonData[0]["coordinates"][0],
        "Polygon",
    )
    targetResult = transform_coordinate(
        3857, int(referencesData.coordSystem), [inputLoc[0], inputLoc[1]], "Point"
    )
    ...
    try:
        ...
        with rasterio.Env(
            ...
            with rasterio.open(dsm) as file:
                out_image, out_transform = mask(
                    file, geoJsonData, crop=True, all_touched=True, nodata=-10000
                )
        out_meta = file.meta
        out_meta.update(
            {
        ...
        dateToFileName = "".join(
            str(datetime.datetime.now()).replace(":", "").replace(".", "").split(" ")
        )
...
def getHighProfile(
    inputLoc, referencesData
):  # 직선 측정 - 종단면 확인 - 거리 프로파일 계산 메소드(매 1m 간격) + 다중 횡단 프로파일(횡단 거리 설정에 따른 여러개의 횡단 좌표 계산)
    ...
    y_changed = []
    transformResult = transform_coordinate(
        3857, int(referencesData.coordSystem), inputLoc[0]["coordinates"], "Polygon"
    )
    ...
    transformResult = transform_coordinate(
        3857, int(referencesData.coordSystem), polygon[0]["coordinates"][0], "Polygon"
    )
    ...
    try:
        ...
        with rasterio.Env(
            ...
            with rasterio.open(dsm) as file:
                out_image, out_transform = mask(
                    file, polygon, crop=True, all_touched=True, nodata=-10000
                )
        ...
        if x[0] == x[1]:  # 세로
            ...
            y_data = np.arange(
                y_changed[0],
                y_changed[1],
                interval if y_changed[0] < y_changed[1] else -interval,
            )
        elif y[0] == y[1]:  # 가로
            ...
            x_data = np.arange(
                x_changed[0],
                x_changed[1],
                interval if x_changed[0] < x_changed[1] else -interval,
            )
        else:
            ...
            x_data = np.arange(
                x_changed[0],
                x_changed[1],
                xDiff if x_changed[0] < x_changed[1] else -xDiff,
            )
        ...
        tmp_x_y = transform_coordinate(
            int(referencesData.coordSystem),
            3857,
            [[x_data[i], y_data[i]] for i in range(len(x_data) - 1)],
            "Polygon",
        )
        tmp_x = []
        tmp_y = []
        ...
        return {
            ...
            "z": z_value,
            "interval": list(
                np.arange(0, math.sqrt(xDis * xDis + yDis * yDis), interval)[:-1]
            ),
        }
    except FileNotFoundError as e:
        print(e)
