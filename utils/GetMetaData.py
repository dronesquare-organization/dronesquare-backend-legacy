...
def getMetaData(tagLabel):
    ...
    if tagLabel['DateTimeOriginal'] is not None:
        originalDate = tagLabel['DateTimeOriginal'].split(' ')[0].replace(':', '-') + ' ' + \
                       tagLabel['DateTimeOriginal'].split(' ')[1]
        metadata['created'] = datetime.datetime.strptime(originalDate, '%Y-%m-%d %H:%M:%S')
    if tagLabel['DateTimeDigitized'] is not None:
        digitizedDate = tagLabel['DateTimeDigitized'].split(' ')[0].replace(':', '-') + ' ' + \
                        tagLabel['DateTimeDigitized'].split(' ')[1]
        metadata['modified'] = datetime.datetime.strptime(digitizedDate, '%Y-%m-%d %H:%M:%S')
