...
def imgSave(param):
    accept, fail, onlyfiles, autoDelta, tmpPath, compressionPath, tmpCompressionPath, tmpThumbnailPath, user, project = param
    for i in range(len(onlyfiles)): # tmp path file name
        if onlyfiles[i].split(".")[-1].lower() in accept:
            ...
            metadata = {"width": im.size[0], "height": im.size[1], "size": os.path.getsize(tmpPath+"/"+onlyfiles[i])}
            tagLabel = {}
            info = im.getexif()

            if info != {} and info is not None:
                fileName = createFileName(compressionPath+"/"+"".join(onlyfiles[i].split(" ")), onlyfiles[i])
                # compression maker
...
def get_s3_path(email, projectId, name):
    ...
    def path_setting_join(path):
        if path is 'original':
            return "{email}/projects/{projectId}/{name}".format(email=email, projectId=projectId, name=name)
        else :
            return "media/{email}/projects/{projectId}/{path}/{name}".format(email=email, projectId=projectId, path=path, name=name)
    return path_setting_join
...
def upload_to_aws(local_name):
    ...
    s3_name = local_name.replace("media", "original").replace('/tmpPath', '').split('original/')[1]
