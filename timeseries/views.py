...
from .serializers import (TimeSeriesInputSerializer,
                          TimeSeriesRelationByProjectIdSerializer,
                          TimeSeriesRelationInputSerializer,
                          TimeSeriesRelationSerializer, TimeSeriesSerializer)
...
class TimeSeriesViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="시계열 그룹 생성",
    ...
    def create(self, request):
        ...
        serializer = TimeSeriesSerializer(data=request.data, context={"request": request})
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="시계열 그룹 이름 변경",
                         request_body=openapi.Schema(
                             type=openapi.TYPE_OBJECT,
                             properties={
                                 "name": openapi.Schema(type=openapi.TYPE_STRING, description="시계열 그룹 이름")
                             }
                         ),
                         manual_parameters=[
                             openapi.Parameter("Authorization", openapi.IN_HEADER, description="Authorization",
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="시계열 그룹 조회",
                         manual_parameters=[
                             openapi.Parameter("Authorization", openapi.IN_HEADER, description="Authorization",
                                               type=openapi.TYPE_STRING)])
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="시계열 그룹 삭제",
                            request_body=openapi.Schema(
                                type=openapi.TYPE_OBJECT, 
                                properties={
...
class TimeSeriesFindByProjectViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(tags=["timeseries-project"],
    ...
    def findByProjectId(self, request, projectId):
        ...
        if len(timeseries) == 0:
            return Response({"message": "empty"})
        serializer = TimeSeriesRelationByProjectIdSerializer(instance=timeseries, many=True)
        return Response(serializer.data)
...
class TimeSeriesRelationViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="시계열 관계 생성",
    ...
    def create(self, request):
        email = str(request.user)
        request.data["email"]=email
        relation = TimeSeriesRelation.objects.filter(timeseriesId=request.data["timeseriesId"],
                                                  projectInfo=request.data["projectInfo"])\
            .aggregate(count_of_relation=Count("projectInfo"))

        if relation["count_of_relation"] != 0:
            return Response({"message": "Already exist"}, status=200)
        serializer = TimeSeriesRelationSerializer(data=request.data, context={"request": request})
        if serializer.is_valid():
            serializer.save()
            relationInfo = TimeSeriesRelation.objects.filter(projectInfo=request.data["projectInfo"]) \
                .aggregate(count_of_relation=Count("projectInfo"))
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="시계열 관계 삭제",
                        request_body=openapi.Schema(
                            type=openapi.TYPE_OBJECT, 
                            properties={
                                'idSet': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_OBJECT, properties={
                                        'groupId':openapi.Schema(type=openapi.TYPE_INTEGER, description='object'),
                                        'relationIdSet':openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Items(type=openapi.TYPE_INTEGER), description='array')
                                    }), description='array'),
                            }
                        ),  
                        manual_parameters=[
    ...
    def group(self, request):
        ...
        for Set in idSet:
            relation = TimeSeriesRelation.objects.filter(id__in=Set["relationIdSet"], email=email, timeseriesId=Set["groupId"])
            if relation.exists():
                relation.delete()
        return Response({"message":"success"})
