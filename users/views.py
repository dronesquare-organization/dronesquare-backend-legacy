...
from .serializers import CustomRegisterSerializer, UserAdditionalSerializer, StorageSerializer, \
    CurrentUserSerializer, UserPasswordFormSerializer, PaymentSerializer, MyTokenObtainPairSerializer
...
class RegistrationViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            ...
            verify_link = FRONTEND_URL + '/email-verify/' + urlsafe_base64_encode(force_bytes(user.pk)).encode().decode()+"/"+token
            ...
            msg = EmailMultiAlternatives(mail_subject, text_content, from_email, [request.data["email"]])
            msg.attach_alternative(html_content, "text/html")
            msg.send()
            return Response({"message": "Confirm your Email"}, status=200)
...
class EmailActiveViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="이메일 인증",
...
class CheckingEmailViewSet(viewsets.ViewSet):
    ...
    permission_classes = [AllowAny]
    @transaction.atomic
    @swagger_auto_schema(operation_description="이메일 중복 확인",
                         request_body=openapi.Schema(
                             type=openapi.TYPE_OBJECT,
                             properties={
...
class UserInfoViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원 정보 가져오기",
                         manual_parameters=[
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원 정보 수정",
    ...
    def create(self, request):
        ...
        query_set = Users.objects.all()
        user = get_object_or_404(query_set, email=email)
        serializer = UserAdditionalSerializer(user, data=request.data, context={"request": request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
...
class UserPasswordViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원 비밀번호 변경",
    ...
    def partial_update(self, request, pk=None):
        if request.data['password'] != request.data['password1']:
            return Response({"message": "Password is not Same"}, status=400)
        user = request.user
        if check_password(request.data['current_password'], user.password):
            ...
            return Response({"message": "current password is not valid"}, status=400)
...
class LogoutView(APIView):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원 로그 아웃",
                         request_body=openapi.Schema(
                             type=openapi.TYPE_OBJECT,
                             properties={
                                 'refresh_token': openapi.Schema(type=openapi.TYPE_STRING, description='리프레쉬 토큰')
                             }
                         ))
...
class StorageDetailViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원 용량 확인",
                         manual_parameters=[
                             openapi.Parameter('Authorization', openapi.IN_HEADER, description="Authorization",
                                               type=openapi.TYPE_STRING)])
    def list(self, request):
...
class PaymentViewSet(viewsets.ViewSet):
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원 모든 계좌 확인",
                         manual_parameters=[
                             openapi.Parameter('Authorization', openapi.IN_HEADER, description="Authorization",
                                               type=openapi.TYPE_STRING)])
    def list(self, request):
    ...
    def create(self, request):
        ...
        if request.data['email'] != email:
            return Response({"message": "unAuthorized"}, status=400)
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원의 해당 계좌 정보 변경",
                         request_body=PaymentSerializer,
                         manual_parameters=[
                             openapi.Parameter('Authorization', openapi.IN_HEADER, description="Authorization",
                                               type=openapi.TYPE_STRING)])
    ...
    @transaction.atomic
    @swagger_auto_schema(operation_description="해당 회원의 해당 계좌 삭제",
                         manual_parameters=[
                             openapi.Parameter('Authorization', openapi.IN_HEADER, description="Authorization",
                                               type=openapi.TYPE_STRING)])
