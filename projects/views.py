...
class ProjectsDetailViewSet(viewsets.ViewSet):
    @transaction.atomic
    @swagger_auto_schema(
        operation_description="프로젝트 세부사항",
        manual_parameters=[
            openapi.Parameter(
                "Authorization",
                openapi.IN_HEADER,
    ...
    def destroy(self, request, pk):
        ...
        if project is not None:
            path = settings.MEDIA_ROOT + "/{email}/projects/{projectId}".format(
                email=email, projectId=pk
            )
...
class DataProcessViewSet(viewsets.ViewSet):
    @transaction.atomic
    @swagger_auto_schema(
        operation_description="데이터 프로세스 신청",
    ...
    def create(self, request):
        email = str(request.user)
        request.data["email"] = email
        serailzer = DataProcessSaveSerializer(
...
class DataProcessFileViewSet(viewsets.ViewSet):
    ...
    def info(self, request, projectId):
        ...
        try:
            ...
            processFiles = (
                DataProcessFile.objects.filter(email=email, dataprocessId=dataprocessId)
    ...
    def create(self, request):
        email = str(request.user)
        processFile = DataProcessFile.objects.filter(
            fileType__in=request.data["fileType"],
            email=email,
            dataprocessId=request.data["dataprocessId"],
        )
